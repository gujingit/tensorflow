...
momentum = tf.Variable(dtype=tf.float32,initial_value=0.0)
...
#sgd-momentum
train = tf.train.MomentumOptimizer(learning_rate=lr,momentum=momentum,use_nesterov=True).minimize(loss)
...
for i in xrange(iter_time):
	...
	t,c=sess.run([train,loss],feed_dict={x:batch_xs,y:batch_ys,lr:learning_rate,momentum:momentum1})
	if i%100 == 0:
...
for i in xrange(iter_time):
	batch_xs, batch_ys = mnist.train.next_batch(100)
	t, c = sess.run([train, loss], feed_dict={x: batch_xs, y: batch_ys, lr: learning_rate, momentum: momentum2})
	if i % 100 == 0:
...
for i in xrange(iter_time):
	batch_xs, batch_ys = mnist.train.next_batch(100)
	t, c = sess.run([train, loss], feed_dict={x: batch_xs, y: batch_ys, lr: learning_rate, momentum: momentum3})
	if i % 100 == 0:
...
plt.plot(xrange(20),acc_train1,'bo-',label='acc_train_momentum1')
plt.plot(xrange(20),acc_test1,'bs-',label='acc_test_momentum1')
plt.plot(xrange(20),acc_train2,'ro-',label='acc_train_momentum2')
plt.plot(xrange(20),acc_test2,'rs-',label='acc_test_momentum2')
plt.plot(xrange(20),acc_train3,'go-',label='acc_train_momentum3')
plt.plot(xrange(20),acc_test3,'gs-',label='acc_test_momentum3')
